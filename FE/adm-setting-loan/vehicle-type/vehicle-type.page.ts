...
export class VehicleTypePage {
  ...
  getListData() {
    let urlApi = this.mainService.getMainUrl() +
      "/api/loan/vehicle-type?page=" + this.indexPaging + "&size=" + this.size;
  ...
  onRemove(data: any) {
    this.swalService.present("Are you sure to delete this Vehicle Type ?", "question", (res: any) => {
      if (res) {
        let urlApi = this.mainService.getMainUrl() +
          '/api/loan/vehicle-type/delete';
  ...
  goToDetail(id: any) {
    ...
    if (id == null) {
      this.navCtrl.navigateForward("/administration/settings/loan/vehicle-type/add-vehicle-type");
    } else {
      this.navCtrl.navigateForward("/administration/settings/loan/vehicle-type/add-vehicle-type", params);
    }
  }
}
