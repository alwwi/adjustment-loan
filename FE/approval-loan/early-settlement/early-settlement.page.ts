...
@Component({
  selector: 'app-early-settlement-approval',
  templateUrl: './early-settlement.page.html',
  styleUrls: ['./early-settlement.page.scss'],
})
export class EarlySettlementPage {
  action: any;
  ...
  getListData() {
    let urlApi = this.mainService.getMainUrl() +
      "/api/user/dataApproval/personal/loan/early-settlement";
    ...
    if (this.loanCategory != undefined && this.loanCategory != '' && this.loanCategory != null) {
      urlApi = urlApi + '&loanCategoryId=' + this.loanCategory;
    }
  ...
  goToDetail(id: any) {
    ...
    this.navCtrl.navigateForward("/request-detail/early-settlement", params);
  }
}
