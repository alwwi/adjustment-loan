...
import com.phincon.talents.app.services.loan.VehicleBrandService;
...
@RestController
@RequestMapping("api/loan/vehicle-brand")
public class VehicleBrandController {
    private static final Logger log = LogManager.getLogger(VehicleBrandController.class);

    @Autowired
    VehicleBrandService service;

    String identifier = "Vehicle Brand";
...
//            endDateUsed=LocalDate.now();
        ...
        return service.listData(page,size,name, startDateUsed,endDateUsed,loanCategoryId,vehicleTypeId);
    }

    @GetMapping("detail")
    public VehicleBrandDetailDTO getDetailEmployeeCertification(@RequestParam(value = "id") String id){
        return service.getDetail(identifier,id);
    }

    @PostMapping
    public ResponseEntity<CustomMessageWithId> submitData(@RequestBody VehicleBrandPostDTO request){
    ...
    @PostMapping("delete")
    public ResponseEntity<CustomMessageWithId> deleteCertification(@RequestBody VehicleBrandPostDTO request){
