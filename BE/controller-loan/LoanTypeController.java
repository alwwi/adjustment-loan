...
public class LoanTypeController {
    private static final Logger log = LogManager.getLogger(LoanTypeController.class);

    @Autowired
    LoanTypeService service;

    String identifier = "Loan Type";
...
//            endDateUsed=LocalDate.now();
    ...
    @GetMapping("detail")
    public LoanTypeDetailDTO getDetailEmployeeCertification(@RequestParam(value = "id") String id){
        return service.getDetail(identifier,id);
    }

    @PostMapping
    public ResponseEntity<CustomMessageWithId> submitData(@RequestBody LoanTypePostDTO request){
    ...
    @PostMapping("delete")
    public ResponseEntity<CustomMessageWithId> deleteCertification(@RequestBody LoanTypePostDTO request){
