...
import com.phincon.talents.app.services.loan.LoanAttachmentService;
...
@RestController
@RequestMapping("api/loan/attachment")
public class LoanAttachmentController {
    private static final Logger log = LogManager.getLogger(LoanAttachmentController.class);

    @Autowired
    LoanAttachmentService service;

    String identifier = "Loan Attachment";
...
//            endDateUsed=LocalDate.now();
    ...
    @GetMapping("detail")
    public LoanAttachmentDetailDTO getDetailEmployeeCertification(@RequestParam(value = "id") String id){
        return service.getDetail(identifier,id);
    }

    @PostMapping
    public ResponseEntity<CustomMessageWithId> submitData(@RequestBody LoanAttachmentPostDTO request){
    ...
    @PostMapping("delete")
    public ResponseEntity<CustomMessageWithId> deleteCertification(@RequestBody LoanAttachmentPostDTO request){
