...
import com.phincon.external.app.dao.loan.LoSyncLoanRequestDtlSalesForceRepository;
...
import com.phincon.external.app.utils.ApplicationPropertiesService;
...
@Service
public class loanAllowanceService extends ParentLoanSyncServices<LoanRequestSyncDTO, LoSyncLoanRequestDtl> {
    Logger log = LoggerFactory.getLogger(loanAdditionalService.class);

    @Autowired
    LoSyncLoanRequestDtlSalesForceRepository requestRepository;
    ...
    void doSync() {
        ...
        //upsert data that need sync
        MapSqlParameterSource mapSqlParameterSource = new MapSqlParameterSource();
        mapSqlParameterSource.addValue("dateUsed",paymentDate);
        mapSqlParameterSource.addValue("secret",applicationPropertiesService.getValueSecret());

        log.info("Begin Add Allowance Data To Sync");
        namedParameterJdbcTemplate.update(requestRepository.updateDataToSyncValue,mapSqlParameterSource);
        namedParameterJdbcTemplate.update(requestRepository.insertDataToSyncValue,mapSqlParameterSource);
        namedParameterJdbcTemplate.update(requestRepository.updateDataToSyncElement,mapSqlParameterSource);
        namedParameterJdbcTemplate.update(requestRepository.insertDataToSyncElement,mapSqlParameterSource);
        log.info("Finish Add Allowance Data To Sync");

        //-----------------------------------------------------------
        // Allowance Bracket
        //-----------------------------------------------------------
        List<Map<String,Object>> allowanceBracketNeedSyncLoanValue = requestRepository.findAllowanceBracketNeedSyncLoanByDate(paymentDate,requestDate);
        List<LoanRequestSyncDTO> allowanceBracketNeedSyncLoanValueDTO = addListMapToDTO(allowanceBracketNeedSyncLoanValue);
        log.info("Allowance Bracket Need Sync : " + allowanceBracketNeedSyncLoanValueDTO.size());
        //-----------------------------------------------------------


        List<LoanRequestSyncDTO> needSyncUsed = new ArrayList<>();
        needSyncUsed.addAll(allowanceBracketNeedSyncLoanValueDTO);
    ...
    @Override
    LoSyncLoanRequestDtl getNewInstance(String requestId) {
        Optional<LoSyncLoanRequestDtl> optReq = requestRepository.findByIdBackup(Long.parseLong(requestId));
        if (optReq.isPresent())
            return optReq.get();
        return new LoSyncLoanRequestDtl();
    }

    public void processResultSalesForce(WrapSalesForceDTOUUID responseFromSalesForce){
        log.info("===============================================");
        log.info("[Loan Allowance] Begin Process Result SalesForce ");
        log.info("===============================================");
        ...
        if(responseFromSalesForce != null && responseFromSalesForce.getResults() != null) {
            for (ResultSyncDTOUUID singleData : responseFromSalesForce.getResults()) {
                ...
                for (String singleRequest : requestNoLists) {
                    ...
                    Optional<LoSyncLoanRequestDtl> dataFound = requestRepository.findById(singleRequest);
                    if (dataFound.isPresent()) {
                        ...
                        log.info(tabAddtional + "Request No (Loan Allowance) Skipped : " + singleRequest + " | SalesForce Id : " + singleData.getId());
                    }
        ...
        log.info("[Loan Allowance] Finish Process Result SalesForce");
        log.info("===============================================");
    }
}
